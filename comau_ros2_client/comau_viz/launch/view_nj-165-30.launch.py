...
def generate_launch_description():
  ...
  xacro_file_path = os.path.join(
    get_package_share_directory('nj-165-30_description'),
    'robots',
    'nj-165-30_robot.urdf.xacro'
  )
   ## If you change, from nj-165-30_description/nj-165-30_upload.launch.py the values ​​of:
   # 'pos_x'       # 'pitch'
   # 'pos_y'       # 'yaw'
   # 'pos_z'       # 'robot_name'
   # 'roll'        # 'use_mimic'
  ...
  robot_upload_launch = IncludeLaunchDescription(
    PythonLaunchDescriptionSource(
      os.path.join(get_package_share_directory('nj-165-30_description'),'launch/nj-165-30_upload.launch.py')
    ),
  )
