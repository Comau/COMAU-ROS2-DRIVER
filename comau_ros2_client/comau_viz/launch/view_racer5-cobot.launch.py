...
def generate_launch_description():
  ...
  xacro_file_path = os.path.join(
    get_package_share_directory('racer5-cobot_description'),
    'robots',
    'racer5-cobot_robot.urdf.xacro'
  )

   ## If you change, from racer5-cobot_description/racer5-cobot_upload.launch.py the values ​​of:
   # 'pos_x'       # 'pitch'
   # 'pos_y'       # 'yaw'
   # 'pos_z'       # 'robot_name'
   # 'roll'        # 'use_mimic'
  ...
  robot_upload_launch = IncludeLaunchDescription(
    PythonLaunchDescriptionSource(
      os.path.join(get_package_share_directory('racer5-cobot_description'),'launch/racer5-cobot_upload.launch.py')
    ),
  )
