...
def generate_launch_description():
  ...
  xacro_file_path = os.path.join(
    get_package_share_directory('nj220_description'),
    'robots',
    'nj220_robot.urdf.xacro'
  )

   ## If you change, from nj220_description/nj220upload.launch.py the values ​​of:
   # 'pos_x'       # 'pitch'
   # 'pos_y'       # 'yaw'
   # 'pos_z'       # 'robot_name'
   # 'roll'        # 'use_mimic'
  ...
  robot_upload_launch = IncludeLaunchDescription(
    PythonLaunchDescriptionSource(
      os.path.join(get_package_share_directory('nj220_description'),'launch/nj220_upload.launch.py')
    ),
  )
