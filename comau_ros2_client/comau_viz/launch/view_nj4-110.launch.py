...
def generate_launch_description():
  ...
  xacro_file_path = os.path.join(
    get_package_share_directory('nj4-110_description'),
    'robots',
    'nj4-110_robot.urdf.xacro'
  )

   ## If you change, from nj4-110_description/nj4-110_upload.launch.py the values ​​of:
   # 'pos_x'       # 'pitch'
   # 'pos_y'       # 'yaw'
   # 'pos_z'       # 'robot_name'
   # 'roll'        # 'use_mimic'
  ...
  robot_upload_launch = IncludeLaunchDescription(
    PythonLaunchDescriptionSource(
      os.path.join(get_package_share_directory('nj4-110_description'),'launch/nj4-110_upload.launch.py')
    ),
  )
